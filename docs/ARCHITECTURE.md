...
### Recommendation response explainability contract

Recommendation responses may include the ADR-021
`RecommendationExplanation` payload documented in
[`docs/recommendation-explainability-api.md`](./recommendation-explainability-api.md).
The explanation payload is generated by backend deterministic Recommendation
Engine logic from scoring facts, transition facts, policy checks, and approved
catalog evidence. It is never generated by LLM song selection: the LLM remains
limited to intent parsing, and selected songs, ordering, warnings, diagnostics,
and explanation facts remain backend-owned. Public clients must render only
audience-authorized fields and must not infer additional facts beyond the
structured reason codes, values, and evidence supplied by the API.
