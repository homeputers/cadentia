        ...
        <LocalizedView><main className="admin-shell" aria-labelledby="import-queue-title">
            ...
            {canReview && (
                <section className="admin-shell__panel" aria-labelledby="bulk-actions-title">
                    ...
                    <button type="button" className="secondary" disabled={!queue || queue.items.length === 0} onClick={toggleSelectAll}>{allSelected ? 'Deselect all on page' : 'Select all on page'}</button>
            ...
            {queue && queue.totalPages > 1 && <nav aria-label="Pagination"><button className="secondary" disabled={filters.page <= 1} onClick={() => setFilters({ ...filters, page: filters.page - 1 })}>Previous</button><button className="secondary" disabled={filters.page >= queue.totalPages} onClick={() => setFilters({ ...filters, page: filters.page + 1 })}>Next</button></nav>}
            ...
            {bulkActionOpen && (
                <div className="admin-dialog-backdrop" role="presentation">
                    <section className="admin-dialog" role="dialog" aria-modal="true" aria-labelledby="bulk-dialog-title">
                        ...
                        <div className="admin-dialog__actions"><button type="button" className="secondary" onClick={() => setBulkActionOpen(false)}>Cancel</button><button type="button" disabled={bulkState === 'saving'} onClick={() => void submitBulk()}>Submit bulk action</button></div>
