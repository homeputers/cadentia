...
import type { CandidateDetail } from '../src/candidate-detail';
...
describe('import candidate detail', () => {
    ...
    it('handles unauthorized and stale-version note states without leaking protected content', async () => {
        ...
        const request = vi.fn().mockImplementation((path: string) => path.endsWith('/notes') ? Promise.reject(Object.assign(new Error('stale'), { status: 412 })) : Promise.resolve(baseDetail));
        const stale = await render(baseDetail, request);
        const textarea = stale.node.querySelector('textarea')!;
        await act(async () => { Simulate.change(textarea, { target: { value: 'Stale note' } } as never); });
        await act(async () => { stale.node.querySelector('form')!.dispatchEvent(new Event('submit', { bubbles: true, cancelable: true })); });
