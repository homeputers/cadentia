...
class RehearsalWorkflowSummaryServiceTest {
    ...
    void summarizesWorkflowCountsSessionsOverridesAndOpenIssues() {
        ...
        workflowReader.status = new WorkflowStatus(
                servicePlanId, ReadinessStateCode.READY, ReadinessStateCode.ISSUES_OPEN, 1, 1);
        ...
        workflowReader.issues = List.of(
                new RehearsalIssueRecord(
                        transitionIssueId,
                        servicePlanId,
                        RehearsalTarget.transition(fromBlockId, toBlockId),
                ...
                new RehearsalIssueRecord(
                        UUID.randomUUID(),
                        servicePlanId,
                        RehearsalTarget.setlistItem(blockId, UUID.randomUUID()),
        ...
        // Act
        var summary = service.summarize(servicePlanId, RehearsalWorkflowSummaryAudience.WORSHIP_LEADER);
    ...
    void filtersSongAndTransitionIssuesByServiceContextAndRedactsPublicDetails() {
        ...
        RehearsalWorkflowSummaryService service = new RehearsalWorkflowSummaryService(
                ...
                Clock.fixed(NOW, ZoneOffset.UTC));
        workflowReader.actions = List.of(
                new RehearsalIssueActionRecord(
                        UUID.randomUUID(),
                        issueId,
                        servicePlanId,
                        IssueActionStatusCode.IN_PROGRESS,
                        "Private owner note", IssueOwnerType.SERVICE_ASSIGNMENT, null, "drums", UUID.randomUUID(), null));
        workflowReader.issues = List.of(
                new RehearsalIssueRecord(
                        issueId,
                        servicePlanId,
                        RehearsalTarget.setlistItem(songBlockId, UUID.randomUUID()),
                ...
                new RehearsalIssueRecord(
                        UUID.randomUUID(),
                        servicePlanId,
                        RehearsalTarget.setlistItem(otherBlockId, UUID.randomUUID()),
                        IssueCategoryCode.DIFFICULT_SONG, IssueSeverityCode.HIGH, IssueStatusCode.OPEN,
        ...
        // Act
        var songIssues = service.songIssues(servicePlanId, songBlockId, RehearsalWorkflowSummaryAudience.PUBLIC);
        var transitionIssues = service.transitionIssues(
                servicePlanId,
                fromBlockId,
                toBlockId,
                RehearsalWorkflowSummaryAudience.ADMIN);
    ...
    private static final class FakeWorkflowReader implements RehearsalWorkflowReader {
        ...
        public WorkflowStatus workflowStatus(UUID servicePlanId) {
            return status == null
                    ? new WorkflowStatus(servicePlanId, ReadinessStateCode.DRAFT, ReadinessStateCode.DRAFT, 0, 0)
                    : status;
        }
    }
}
