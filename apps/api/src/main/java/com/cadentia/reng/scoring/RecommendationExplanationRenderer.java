...
public class RecommendationExplanationRenderer {
    ...
    private static final Map<String, String> TEMPLATE_BY_KEY = Map.ofEntries(
            ...
            Map.entry("item.score_component_team_suitability", "Team-suitability component scored {score}."),
            Map.entry("team.required_instrument_coverage", "Required team coverage for {targetCode}: {actualCount}/{requiredCount} ({status})."),
            Map.entry("team.optional_instrument_fit", "Optional team fit for {targetCode}: {actualCount}/{requiredCount} ({status})."),
            Map.entry("team.vocal_configuration", "Vocal configuration {vocalConfiguration}: {actualCount}/{requiredCount} covered ({status})."),
            Map.entry("team.lead_vocal_range_fit", "Lead-vocal range requirement is {rangeRequirement}; fit count {fitCount} ({status})."),
            Map.entry("team.skill_level_floor", "Skill floor for {targetCode} rank {minimumSkillRank}: {actualCount}/{requiredCount} covered ({status})."),
            Map.entry("team.availability_status", "Availability status has {unavailableCount} unavailable assignment(s) and {activeAssignmentCount} active assignment(s) ({status})."),
            Map.entry("team.assignment_status", "Assignment status has {assignedCount} assigned and {activeAssignmentCount} active ({status})."),
            Map.entry("team.readiness_warning", "Team readiness warnings: {warningCount} ({status})."),
